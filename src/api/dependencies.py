...
class _SharedComponents:
    ...
    knowledge_refiner: KnowledgeRefiner | None
...
def _build_shared_components(settings: Settings) -> _SharedComponents:
    ...
    if settings.hyde_enabled:
        ...
        if settings.hyde_confidence_threshold is not None:
            _LOG.info(
                "HyDE confidence gating enabled: threshold=%.3f",
                settings.hyde_confidence_threshold,
            )
    else:
    ...
    if settings.routing_enabled:
        query_router: QueryRouter | None = QueryRouter()
...
def build_async_pipeline() -> tuple[AsyncRAGPipeline, ModelLoader, AsyncQdrantClient]:
    ...
    async_client = AsyncQdrantClient(url=settings.qdrant_url, api_key=shared.api_key_str)
